...
export function SignedAsset() {
  return <><p className="eyebrow">An asset and its credentials</p><div className="title-row"><h1>The pixels. <em>The context.</em></h1><span className="mono sample-label">ILLUSTRATIVE DATA</span></div><div className="asset-layout"><AssetImage src={assets.signedAsset} alt="Example asset associated with the illustrative manifest" label="Signed asset"/><div className="credential-panel"><div className="credential-heading"><span className="cc-mark">cr</span><h2>Content Credentials</h2></div><dl><dt>ASSET</dt><dd>{exampleManifest.asset}</dd><dt>CLAIM GENERATOR</dt><dd>{exampleManifest.generator}</dd><dt>RECORDED ACTIONS</dt><dd className="mono">{exampleManifest.actions.join(' → ')}</dd><dt>SIGNER</dt><dd>{exampleManifest.signer}</dd></dl><p className="validation-note">{exampleManifest.validation}</p></div></div></>;
}
